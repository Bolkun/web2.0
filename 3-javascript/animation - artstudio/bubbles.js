...
function PointCollection() {
    ...
    this.update = function () {
        for (var i = 0; i < this.points.length; i++) {
            ...
            var dd = (dx * dx) + (dy * dy);
            var d = Math.sqrt(dd);
			//радиус тряска кружков 
            point.targetPos.x = d < 50 ? point.curPos.x - dx : point.originalPos.x;
            point.targetPos.y = d < 50 ? point.curPos.y - dy : point.originalPos.y;
...
function drawName(name, letterColors) {
 ...
 //New
    ...
    for (var j = 0; j < g.length; j++) {
        ...
        g[j].curPos.y = (canvasHeight / 2 - 105 ) + g[j].curPos.y;					//передвижение анимации низ по у
