	...
	<div data-role="page">
		...
		<div data-role="content">
			<div style="text-align: center">
				...
				?>
				<button style="text-align: center" id="aufgabe2" data-inline="true" data-role="button">B&#8594;A</button>
				</form>
			</div>
			<?php
						...
						<td class="column1">
							<?php
								...
								for($e=0; $e<$anz_zeile; $e=$e+1){
									...
									$array_sprache1[$e]=$tmp_array[0];
									//echo "$e $array_sprache1[$e]";	
									$array_aussprache1[$e]=$tmp_array[1];
									$array_sprache2[$e]=$tmp_array[2];
									$array_aussprache2[$e]=$tmp_array[3];
								}
								
							    /*Frage list erstellen*/
								$f = fopen("list/tmp/frage.txt", 'a+');	//a+ (lesen + schreiben)
								...
								if($aufgabe_anz == 0){
									for($a=0; $a<$anz_zeile; $a=$a+1){
										fwrite($f, $array_sprache1[$a] .PHP_EOL);	
									}
								}
								//Ablesen der File in array
								$file_out = file("list/tmp/frage.txt");
								//Anzahl der Zeile in Datei(beginnend von 1)
								$anz_zeile_frage = count($file_out);	
								for($q=0; $q<$anz_zeile_frage; $q=$q+1){
									//echo $file_out[$q];
									$tmp_array_frage[$q] = $file_out[$q];
									//echo "$q $tmp_array_frage[$q]";
								}
								fclose($f);
								//Aufgabe erstellen
								$zufall_s1 = $anz_zeile_frage-1;	//starte von letzte Element
								file_put_contents("list/tmp/frage.txt", $file_out[$zufall_s1] , FILE_APPEND); //schreibt die gewohlte zeile in Datei
								unset($file_out[$zufall_s1]);	//löscht die Zeile
								file_put_contents("list/tmp/frage.txt", implode("", $file_out)); //der Rest in Datei schreiben
